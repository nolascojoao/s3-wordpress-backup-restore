...
exec > >(tee -a /opt/backups/logs/backup.log) 2>&1
...
if ! docker exec mysql test -f /root/.my.cnf; then
    echo -e "[client]\nuser=root\npassword=${MYSQL_ROOT_PASSWORD}" | docker exec -i mysql tee /root/.my.cnf > /dev/n>
fi
...
echo "Backup completo e enviado para o S3! $(date '+%Y-%m-%d %H:%M:%S')"
